...
st.markdown(
    f"""
    <style>
    /* ... (CSS styles for buttons, containers, metrics - keep the previous block) ... */
    </style>
    """,
...
def run_ai_generation(prompt_text: str, uploaded_file: BytesIO = None, max_tokens=1500, temp=0.5):
    # ... (AI function body remains the same, relying on imported 'client') ...
    if not client:
...
# --- INITIALIZATION BLOCK (CRITICAL FIX) ---
...
def render_main_navigation_sidebar():
    ...
    with st.sidebar:
        ...
        for item in menu_options:
            ...
            # NOTE: We use st.button, and the CSS applies the active style
                ...
                st.markdown(
                    ...
                    <script>
                        ...
                        buttons.forEach(function(btn) {{
                            // Targeting the specific button by its visible label text
...
def render_usage_dashboard():
    ...
    # ... (Content remains the same as in previous response, with Unlimited fix) ...
    ...
    if storage['tier'] == 'Unlimited':
        ...
        # Set a reasonable max for visual comparison if desired, or skip the bar/chart entirely.
...
def render_plan_manager():
    ...
    col_list = list(st.columns(4))
    
    tier_order = ["Free Tier", "28/1 Pro", "Teacher Pro", "Universal Pro", "Unlimited"]
    
    for i, tier in enumerate(tier_order):
        price = TIER_PRICES[tier]
        limit = f"{TIER_LIMITS[tier] / 1000:.0f} GB" if TIER_LIMITS[tier] > 500 else f"{TIER_LIMITS[tier]} MB"
        
        with col_list[i % 4]:
            with st.container(border=True):
                st.markdown(f"#### {tier}")
                st.markdown(f"**{price}**")
                st.markdown(f"Storage: *{limit}*")
